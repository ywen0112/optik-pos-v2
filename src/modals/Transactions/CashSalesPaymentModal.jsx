...
import { NewCashSalesPayment, NewCashSalesPaymentDetail, SaveCashSalePayment } from '../../api/transactionapi';
...
const CashSalesPaymentModal = ({ isOpen, onClose, total, companyId, userId, cashSalesId, onError, onSave }) => {
  const [selectedPayments, setSelectedPayments] = useState([]);
  const [activePaymentMethod, setActivePaymentMethod] = useState(null);
  const [cashSalesPaymentData, setCashSalesPaymentData] = useState(null);
  ...
  const createNewCashSalesPayment = async () => {
    try {
      const response = await NewCashSalesPayment({ companyId, userId, id: cashSalesId });
      setCashSalesPaymentData(response.data);
    ...
    try {
      ...
      if (response?.success && response.data) {
        const newEntry = {
          ...
          amount: '',
          cashSalesPaymentDetailId: response.data.cashSalesPaymentDetailId,
          reference: '',
          refno: '',
          ccno: '',
          approvalcode: '',
...
export default CashSalesPaymentModal;
