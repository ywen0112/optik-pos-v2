  ...
  return (
    <div className="flex h-screen w-screen items-center justify-center bg-gray-900">
      <div className="w-full max-w-lg bg-white p-8 rounded-lg shadow-lg">
        <h2 className="text-xl font-bold text-center">Select Company</h2>
        <p className="text-sm text-center text-gray-500">Select a company to proceed</p>
        <div className="grid grid-cols-2 gap-4 mt-6">
          {companies.map((company) => (
            <div key={company.customerId} className="flex justify-between items-center bg-gray-100 p-4 rounded-lg shadow-md cursor-pointer hover:bg-gray-200" onClick={() => handleCompanySelect(company)}>
        ...
        <button className="mt-6 w-full bg-gray-700 text-white py-2 rounded-md" onClick={handleCancel}>Cancel</button>
      </div>
    </div>
  );
};
