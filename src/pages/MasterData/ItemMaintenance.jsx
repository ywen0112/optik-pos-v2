  ...
  return (
    <div>
        ...
        <div className="text-right">
            <button className="bg-secondary text-white px-4 py-1 rounded text-xs hover:bg-secondary/90 transition" onClick={handleAddItem}
            >
            Add Item
            </button>
        </div>
            ...
            <table className="w-full border-collapse">
                <thead className="bg-gray-200 border-b-2 border-gray-100 font-bold text-xs text-secondary text-left">
                    <tr>
                        ...
                        <th className="py-3">DESC 2</th>
                        <th className="py-3">ITEM GROUP</th>
                        <th className="py-3">ITEM TYPE</th>
                        <th className="py-3">ACTIONS</th>
                    </tr>
                </thead>
                <tbody>
                {items.map((item, index) => (
                    <tr key={item.itemId} className="text-xs font-medium text-secondary border-gray-100">
                    <td className="pl-4 p-2">
                        {(pagination.currentPage - 1) * pagination.itemsPerPage + index + 1}
                    </td>
                    <td className="p-1">{item.itemCode ?? "-"}</td>
                    <td className="p-1">{item.description ?? "-"}</td>
                    <td className="p-1">{item.desc2 ?? "-"}</td>
                    <td className="p-1">{getItemGroupLabel(item.itemGroupId)}</td>
                    <td className="p-1">{getItemTypeLabel(item.itemTypeId)}</td>
                    <td className="p-1 flex space-x-1">
                        <button className="text-blue-600 pl-0" onClick={() => handleOpenModal(item, "view")}><Eye size={14} /></button>
                        <button className="text-yellow-500 pl-0" onClick={() => handleOpenModal(item, "edit")}><Pencil size={14} /></button>
                        <button className="text-red-500 pl-0" onClick={() => handleDeleteClick(item.itemId)}><Trash2 size={14} /></button>
                    </td>
                    </tr>
                ))}
                </tbody>
